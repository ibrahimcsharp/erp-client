...
export class RoleService {
  ...
  GetBuyerWiseUIListPermissionByUserId(id: number, userName: string ) {
    return this.http.get<any[]>(
      this.baseUrl_ + "RoleManage/GetBuyerWiseUIListPermissionByUserId?userId=" + id + "&userName="+ userName,
      {
        headers: this.token.headerToken(),
      }
    );
  ...
  CreateBuyersWiseMenuListPermission(list: any) {   

    var body = {     
      buyerWiseMenuPermissionList: list,
    };
    // console.log(body);
    return this.http.post(
      this.baseUrl_ + "RoleManage/CreateBuyersWiseMenuListPermission",
      body,
      {
        headers: this.token.headerToken(),
      }
