...
@Component({
  selector: 'app-cost-submit-import-list',
  templateUrl: './cost-submit-import-list.component.html',
  styleUrls: ['./cost-submit-import-list.component.scss']
})
export class CostSubmitImportListComponent implements OnInit {
  ...
  LoadProccessedChallan() {
    this.challanService.getImportChallans().subscribe(
      (data: any[]) => {
        this.ChallanModelList = data.filter(x => x.status == 'IMPORT_COSTING_PROCESSED');
      },
      (error) => {
